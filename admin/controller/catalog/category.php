...
class ControllerCatalogCategory extends Controller {
	...
	protected function getList() {
		...
		$data['sort_type'] 		 = $this->url->link('catalog/category', 'token=' . $this->session->data['token'] . '&sort=type' . $url, true);
	...
	protected function validateForm() {
		...
		//分类类型验证
		$inttype = intval($this->request->post['type']);
		if ($inttype < 1 || $inttype > 2 || is_null($inttype)) {
